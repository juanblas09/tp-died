...
;
	...
	public List<List<Vertex<T>>> findAllPaths(T source, T destination) {
	    Vertex<T> sourceVertex = getNodo(source);
	    Vertex<T> destinationVertex = getNodo(destination);
	    List<List<Vertex<T>>> allPaths = new ArrayList<>();
	    List<Vertex<T>> currentPath = new ArrayList<>();
	    currentPath.add(sourceVertex);
	    
	    findAllPathsRecursive(sourceVertex, destinationVertex, currentPath, allPaths);
	    
	    return allPaths;
	}

	
	private void findAllPathsRecursive(Vertex<T> current, Vertex<T> destination, List<Vertex<T>> currentPath, List<List<Vertex<T>>> allPaths) {
	    if (current.equals(destination)) {
	        // Found a path to the destination, add a copy to the list of all paths
	    ...
	    for (Vertex<T> neighbor : neighbors) {
	        if (!currentPath.contains(neighbor)) {
	            ...
	            findAllPathsRecursive(neighbor, destination, currentPath, allPaths);
